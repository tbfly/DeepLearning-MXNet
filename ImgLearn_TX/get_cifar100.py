## Download cifar100 and extract the image files(.png)

## put this file into the path: mxnet-root/example/notebooks/
## open terminal and type: python get_cifar100.py
...
def extractData(input_file, phase, fine_label_names, coarse_label_names, data_dir):
    """
    ...
    fine_dirname = os.path.join(data_dir, 'cifar-100-python', 'fine', phase)
    os.makedirs(fine_dirname)
    coarse_dirname = os.path.join(data_dir, 'cifar-100-python', 'coarse', phase)
    os.makedirs(coarse_dirname)

    with open(os.path.join(data_dir, 'cifar-100-python', 'fine', '%s.txt' % phase), 'w') as fine_textfile, \
    open(os.path.join(data_dir, 'cifar-100-python', 'coarse', '%s.txt' % phase), 'w') as coarse_textfile:
    ...
    for fine, coarse in fine_to_coarse.iteritems():
        os.system("mkdir -p " + os.path.join(coarse_dirname, coarse))
        os.symlink(
...
if not os.path.isdir(data_dir):
    os.system("mkdir" + data_dir.split('/')[0])
    os.system("mkdir -p " + data_dir)
os.chdir(data_dir)
if (not os.path.exists('train.rec')) or \
   (not os.path.exists('val.rec')):
    os.system("wget https://www.cs.toronto.edu/~kriz/cifar-100-python.tar.gz")
    os.system("tar -zxf cifar-100-python.tar.gz")
    os.system("rm cifar-100-python.tar.gz")
os.chdir("../..")
...
label_filename = 'meta'
label_filepath = os.path.join(data_dir, 'cifar-100-python', label_filename)
...
for level, label_names in [('fine', fine_label_names),('coarse', coarse_label_names),]:
    dirname = os.path.join(data_dir, 'cifar-100-python', level)
    os.system("mkdir -p " + dirname)
...
for filename, phase in [
    ...
    filepath = os.path.join(data_dir, 'cifar-100-python', filename)
    assert os.path.exists(filepath), 'Expected "%s" to exist' % filename

    extractData(filepath, phase, fine_label_names, coarse_label_names, data_dir)
